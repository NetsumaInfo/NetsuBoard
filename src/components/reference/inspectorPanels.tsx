...
import { cn, fmtNumber, parseDecimal, uiLocale } from "@/lib/utils";
...
function TrimNum({ value, empty, placeholder, ariaLabel, onCommit }: {
  ...
  const shown = draft ?? (empty ? "" : fmtNumber(value, { maximumFractionDigits: 1, useGrouping: false }));
  return (
    <input
...
export function TrimControls({ item }: { item: BoardItem }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <TrimNum
        value={hi} empty={item.trimOut == null} placeholder={dur ? fmtNumber(dur, { maximumFractionDigits: 1, useGrouping: false }) : undefined}
