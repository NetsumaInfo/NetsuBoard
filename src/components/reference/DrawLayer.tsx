...
const HitTargets = memo(function HitTargets({ shapes, hitW, onDown, onMove, onUp }: {
  ...
  return (
    <>
      {shapes.map((s) => {
        const handlers = {
          // `data-draw-shape` is read by the board context menu, which selects the aimed shape
          // before opening — same contract as `data-board-item` for items.
          "data-draw-shape": s.id,
