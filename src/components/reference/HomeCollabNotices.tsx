...
type Invite = { inviteId: string; projectId: string; role: string; from: Profile };
type InboxNotice = {
  projectId: string;
  actors: number;
...
export function HomeCollabNotices({ onOpenScene, onCardStatus }: {
  ...
  async function join(invite: Invite) {
    ...
    try {
      ...
      const saved = await nr.reference?.saveScene({
        name: invite.from.name || invite.from.handle || t("home.sharedBoard"),
        items: [],
        view: null,
        collaboration: { projectId: result.projectId },
      });
      if (!saved?.ok || !saved.id) throw new Error(saved?.error || t("home.invites.failed"));
      onOpenScene(saved.id);
    } catch (error) {
      fail(error);
    } finally {
      setBusy(null);
  ...
  const visibleInvites = (invites ?? []).filter((invite) => !snoozed.has(invite.inviteId));
