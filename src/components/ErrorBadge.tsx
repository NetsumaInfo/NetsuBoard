...
// erreur est journalisée (console.error, exception non gérée, rejet de promesse, ou log du core de
...
import { useSettingsUi } from "@/components/settings/useSettingsUi";
import {
...
export function ErrorBadge() {
  ...
  const onConsole = useSettingsUi((s) => s.open && s.section === "console");
    ...
    <ContextMenu>
      <ContextMenuTrigger
        render={
          <button
            type="button"
            onClick={() => { openSettings("console"); markConsoleSeen(); }}
      ...
      <ContextMenuContent className="min-w-44">
        <ContextMenuItem onClick={() => { openSettings("console"); markConsoleSeen(); }}><Terminal /> {t("errorBadge.open")}</ContextMenuItem>
