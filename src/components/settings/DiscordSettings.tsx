  ...
  return (
    <section className="mt-8 flex flex-col gap-4">
      ...
      {/* Un seul cadre pour les trois lignes : elles règlent la MÊME chose (ce que la carte affiche).
          Trois cartes séparées feraient passer une famille pour trois sujets. */}
      <div className={cn("divide-y divide-border rounded-lg border border-border transition-opacity", off && "opacity-50")}>
        ...
        <Row title={t("discord.showBoard")} hint={t("discord.showBoardHint")}>
          <Toggle
            size="sm" variant="outline" pressed={prefs.showBoard} onPressedChange={(v) => patch({ showBoard: v })}
            disabled={off} className={TOGGLE_CLASS}
          >
            {prefs.showBoard ? t("discord.on") : t("discord.off")}
          </Toggle>
        </Row>

        <Row title={t("discord.showItems")}>
          <Toggle
            size="sm" variant="outline" pressed={prefs.showItems} onPressedChange={(v) => patch({ showItems: v })}
            disabled={off} className={TOGGLE_CLASS}
          >
            {prefs.showItems ? t("discord.on") : t("discord.off")}
          </Toggle>
        </Row>
