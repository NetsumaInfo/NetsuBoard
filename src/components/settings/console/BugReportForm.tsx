...
// NetsuRush dérivait cette liste de sa navigation ; NetsuBoard n'a qu'une page, donc elle est écrite
// ici. Ce sont les endroits d'où un rapport peut réellement partir — pas des fonctionnalités.
const MODULES = ["board", "updates", "console", "settings", "setup"] as const;
const DEFAULT_MODULE: (typeof MODULES)[number] = "board";
...
// Une seule colonne sous 640 px : la fenêtre des Paramètres se rétrécit avec celle de l'app, où deux
// colonnes fixes débordent.
...
// Ce que la machine sait, elle le remplit (specs, compte Discord, journal) ; le reste est demandé.
// Rien de ce qui a été écrit n'est perdu si l'envoi échoue : le rapport se télécharge.
export function BugReportForm() {
  const { t, i18n } = useTranslation("settings");
  const activeHost = useApp((s) => s.activeHost);
  const connected = useApp(hostConnected);
  const { context } = useBugContext();
  ...
  const [moduleId, setModuleId] = useState<string>(DEFAULT_MODULE);
  ...
  function buildRequest(files: BugReportRequest["attachments"], relay: BugReportRequest["relay"] = null): BugReportRequest {
    return {
      ...
      module: moduleId,
      moduleLabel: label(`bugReport.modules.${moduleId}`),
  ...
  return (
    <section className="flex flex-col gap-5">
      <header>
        ...
        <p className="mt-1 text-xs text-muted-foreground">{t("bugReport.subtitle")}</p>
      </header>

      {configured === false && (
        <p className="rounded-md border border-[var(--color-warn)]/40 bg-[var(--color-warn)]/10 px-3 py-2 text-xs text-[var(--color-warn)]">
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        <Row>
          ...
          <Field label={t("bugReport.module")}>
            <Select value={moduleId} onValueChange={(v) => setModuleId(String(v))} items={MODULES.map((m) => ({ value: m, label: t(`bugReport.modules.${m}`) }))}>
              <SelectTrigger><SelectValue>{t(`bugReport.modules.${moduleId}`)}</SelectValue></SelectTrigger>
              <SelectContent>
                {MODULES.map((m) => <SelectItem key={m} value={m}>{t(`bugReport.modules.${m}`)}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
        </Row>
        ...
        <div className="mt-1 flex flex-col gap-4 border-t border-border/60 pt-5">
          ...
          {/* L'instantané machine est joint AUTOMATIQUEMENT et se lit en entier dans la Console : le
              dupliquer ici allongerait le formulaire sans rien apprendre. La saisie libre n'apparaît
              donc que quand le service n'a rien pu lire. */}
          {context ? (
            <p className="text-xs text-muted-foreground">{t("bugReport.specs.attached")}</p>
          ) : (
            <Field label={t("bugReport.specs.manual")} htmlFor="bug-specs">
              <Textarea id="bug-specs" value={manualSpecs} onChange={(e) => setManualSpecs(e.target.value)} rows={2} placeholder={t("bugReport.specs.manualPlaceholder")} />
            </Field>
          )}
