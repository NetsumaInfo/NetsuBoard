...
// Unité rendue par Intl plutôt qu'écrite en dur : « Mo » n'est juste qu'en français, et une taille
// de fichier n'a pas à devenir une clé de traduction par langue.
            ...
            {previews.map(({ file, url }) => {
              ...
              return (
                <li key={`${file.name}-${file.lastModified}`} className="group relative overflow-hidden rounded-md border border-border bg-input/30">
                  ...
                  <p className="truncate px-1.5 pb-1 text-[10px] text-muted-foreground">{sizeLabel(file.size, i18n.language)}</p>
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <button
