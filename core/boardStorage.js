...
// Paramètres › Stockage : distinguer ce que l'app peut rendre de ce qu'elle est seule à détenir.
//
// Trois magasins, et c'est toute la difficulté :
//
//   - les CACHES (vignettes, proxies) se refabriquent depuis la source. Les vider ne coûte qu'un
//     peu de temps au prochain affichage : aucune vérification n'est nécessaire.
//
//   - le MAGASIN D'ASSETS (`reference/assets`) n'est PAS un cache, malgré les apparences. Il reçoit
//     l'image collée, le média récupéré d'un lien, les frames extraites, la sortie d'un upscale.
//     Pour un board resté dans la bibliothèque, ces octets sont la SEULE copie qui existe au monde.
//
...
//
// RECONNAÎTRE LE DOUBLE SANS RELIRE UN OCTET. Un asset s'appelle `<md5>.<ext>` (reference.js#saveAsset)
// et son adoption le nomme `<lisible>-<md5 sur 12>.<ext>` (sidecar.js#placeFor, via `fingerprint`,
// qui reprend l'empreinte déjà portée par le nom au lieu de relire le fichier). Lire les noms du
// dossier compagnon suffit donc à savoir qui est déjà rangé ailleurs ; la taille le confirme.
// Un asset au nom LISIBLE (sortie d'upscale) n'a pas cette chance : son empreinte de rangement est
// un sha256, qu'on ne calcule que si un fichier compagnon a exactement sa taille.
//
...
// même travail en parallèle borné tient sous la seconde ; la borne évite d'ouvrir des milliers de
...
/** Poids d'un dossier de cache. Voir `STAT_BATCH` : la version sérielle partagée est trop lente ici. */
async function measureDir(dir, depth = 3) {
  const stats = await statAll(await listFiles(dir, depth));
  return stats.reduce((total, entry) => total + entry.stat.size, 0);
}
...
function createBoardStorage({ refStore, netsu }) {
  ...
  async function projectCopies() {
    ...
    for (const entry of recents.list('board')) {
      ...
      for (const found of await statAll([...wanted.keys()])) {
        ...
        byHash.set(wanted.get(found.file), { project: entry.path, file: found.file, bytes: found.stat.size });
   ...
   * @returns {{ used: Map<string, { id: string, name: string, collaborative: boolean }[]>, live: Set<string> }}
   */
  ...
  /**
   * L'état du stockage, tel que le panneau l'affiche.
  ...
  async function audit(opts) {
    try {
      ...
      const caches = await Promise.all(
        [['thumb', getThumbDir()], ['proxy', getProxyDir()]]
          .map(async ([kind, dir]) => ({ kind, dir, bytes: await measureDir(dir) })),
      );
      return {
        ok: true,
  ...
  /**
   * Libère. Le renderer demande une PORTÉE, jamais des chemins : la liste des fichiers effacés est
   * recalculée ici, à l'instant de la suppression. Un asset devenu utile entre l'affichage du
   * panneau et le clic n'est donc pas emporté par une classification périmée.
   * @param {{ caches?: boolean, assets?: boolean, liveRefs?: string[] }} [opts]
   */
  async function free(opts) {
    ...
    try {
      ...
      if (options.caches) {
        for (const clean of [thumbs.cleanThumbs, proxy.cleanProxies]) {
          try {
            const result = await clean();
            bytes += Number(result && result.bytes) || 0;
            files += Number(result && result.files) || 0;
          } catch (_) { /* un cache indisponible n'empêche pas de vider l'autre */ }
        }
      }
   ...
   * toute façon la vraie empreinte de contenu, donc un homonyme de hasard (48 bits + même
   * extension privilégiée) ne peut au pire qu'exposer un autre média de l'utilisateur — visible
   * immédiatement sur le board — jamais corrompre un document.
   ...
   * dossier) — le soin par empreinte s'arrête là où les octets ont vraiment disparu.
   * @param {{ refs: unknown, projectPath?: string }} opts
