...
// via NR_HOME ; défaut = %LOCALAPPDATA%\NetsuBoard (Windows) ou ~/.netsuboard (autres).
//
// Dossier DISTINCT de celui de NetsuRush, et ce n'est pas cosmétique : les deux applications
// s'installent côte à côte, et un dossier partagé ferait lire à l'une la configuration de l'autre
// (chemins de runtime, cache, assets du board) alors qu'elles n'embarquent pas les mêmes moteurs.
const NR_HOME = process.env.NR_HOME
  || (process.platform === 'win32'
    ? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), 'AppData', 'Local'), 'NetsuBoard')
...
// yt-dlp AUTONOME (exécutable qui embarque son propre interpréteur), posé par le setup dans
// NR_HOME/runtime/bin. C'est ce qui remplace `python -m yt_dlp` : NetsuBoard n'installe aucun
// environnement Python, et il serait absurde d'en exiger un pour résoudre un lien.
//
// `ytDlpCommand()` rend la paire { bin, args } à préfixer, pour que les appelants n'aient pas à
// connaître laquelle des deux formes est disponible. Repli sur le module Python (utile en dev, où
// un venv traîne souvent) puis sur `yt-dlp` du PATH.
function ytDlpCommand() {
  const configured = CONFIG.ytDlp || path.join(NR_HOME, 'runtime', 'bin', process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp');
  try { if (fs.existsSync(configured)) return { bin: configured, args: [] }; } catch (_) {}
  if (CONFIG.python) return { bin: CONFIG.python, args: ['-m', 'yt_dlp'] };
