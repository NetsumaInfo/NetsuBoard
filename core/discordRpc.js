...
/** @typedef {{ enabled:boolean, showBoard:boolean, showItems:boolean, showElapsed:boolean, showLinks:boolean, detailsTpl:string, stateTpl:string }} DiscordPrefs */
...
/** Substitue {board} / {items} dans un gabarit utilisateur. */
function fillTemplate(tpl, ctx) {
  return String(tpl)
    .replace(/\{board\}/g, ctx.board || '')
...
function createDiscordRpc({ CONFIG, broadcast, dataDir }) {
  ...
  function buildActivity(force) {
    ...
    const details = prefs.detailsTpl.trim()
      ? clampText(fillTemplate(prefs.detailsTpl, { board: boardLabel, items: itemsLabel }))
      : clampText(boardLabel);
    const state = prefs.stateTpl.trim()
      ? clampText(fillTemplate(prefs.stateTpl, { board: boardLabel, items: itemsLabel }))
      : clampText(itemsLabel);
