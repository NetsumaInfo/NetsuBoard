...
const REPO_URL = 'https://github.com/NetsumaInfo/NetsuBoard';
...
/** @typedef {{ enabled:boolean, showBoard:boolean, showItems:boolean, showElapsed:boolean, showLinks:boolean }} DiscordPrefs */

// NetsuBoard n'a qu'une page : il n'y a pas de « module ouvert » à annoncer. Le contexte utile est
// donc le board lui-même — son nom, et combien de références il porte.
/** @type {DiscordPrefs} */
const DEFAULT_PREFS = {
  enabled: true,
  showBoard: false, // vie privée : un nom de board peut trahir un client → opt-in explicite
  showItems: true,
  showElapsed: true,
  showLinks: true,
};
...
function sanitizePrefs(raw) {
  ...
  for (const k of /** @type {const} */ (['enabled', 'showBoard', 'showItems', 'showElapsed', 'showLinks'])) {
...
function createDiscordRpc({ CONFIG, broadcast, dataDir }) {
  ...
  // What goes into `assets.large_image`: the `nb_logo` asset key once it is published, the application
  // icon url while it is not, `null` when neither is known (offline boot) — a card with no art.
  /** @type {string | null} */
  let largeImage = LARGE_IMAGE;

  // `items` arrive DÉJÀ formaté et traduit par le renderer (« 12 références ») : les locales vivent
  // dans `src/locales/`, les recopier ici ferait deux catalogues à tenir pour une seule ligne.
  /** @type {{ board:string|null, items:string|null }} */
  let ctx = { board: null, items: null };
  ...
  function buildActivity(force) {
    ...
    const boardLabel = prefs.showBoard ? (ctx.board || '') : '';
    const itemsLabel = prefs.showItems ? (ctx.items || '') : '';
    // The lines are ours, never free text: a presence carries the app's name, so whatever it says is
    // read as coming from NetsuBoard.
    const details = clampText(boardLabel);
    const state = clampText(itemsLabel);

    /** @type {Record<string, any>} */
    const activity = {};
    // Art, and the only thing `large_url` can hang on. `nb_logo` is used once it is published in the
    ...
    if (largeImage) activity.assets = { large_image: largeImage, large_text: 'NetsuBoard' };
  ...
  /** @param {{ board?:string|null, items?:string|null }} next */
  function setContext(next) {
    const board = next?.board ?? null;
    const items = next?.items ?? null;
    if (board === ctx.board && items === ctx.items) return snapshot();
    ctx = { board, items };
    push();
    emit(); // l'aperçu des Paramètres suit le board ouvert, sans attendre le throttle de 15 s
    return snapshot();
  }
