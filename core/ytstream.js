...
function runYtdlp(id, cookiesBrowser, cookiesFile) {
  return new Promise((resolve) => {
    ...
    child.on("error", (e) => {
      ...
      resolve({
        ok: false,
        error: missing ? t("ytdlpMissing", { path: ytdlp.bin }) : String(e),
    ...
    child.on("close", (code) => {
      ...
      // `--print duration` rend des secondes, entieres ou decimales ; NA pour un direct.
...
/**
 * Durée d'une vidéo YouTube, en secondes, ou null. Sert le sélecteur de portée du board : sans elle,
 * la seule source était l'index du conteneur lu par le `<video>`, donc de longues secondes d'attente
