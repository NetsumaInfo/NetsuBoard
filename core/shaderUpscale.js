...
function writeFrame(bin, input, time, out, filter, complex) {
  ...
  return new Promise((resolve) => {
    execFile(bin, args, { cwd: SHADER_DIR, timeout: FRAME_TIMEOUT_MS }, (err, _out, stderr) => {
      ...
      if (!size) return resolve({ ok: false, error: t('ffmpegNoImageWritten', { name: path.basename(out) }) });
      resolve({ ok: true });
    });
  });
}
...
async function runShaderFrame(opts) {
  ...
  const up = alpha
    ? await writeFrame(bin, input, at, out, null, alphaGraph(filter, ow, oh))
    : await writeFrame(bin, input, at, out, filter);
