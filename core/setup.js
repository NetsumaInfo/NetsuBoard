...
// Moteur d'upscale prêt = au moins un shader GLSL posé. On compte les fichiers plutôt que d'en
// nommer un : le catalogue de shaders évolue (cf. core/shaderUpscale.js), le dossier reste.
function shadersReady(config = CONFIG) {
  const dir = config.shaderDir;
  if (!exists(dir)) return false;
  try { return fs.readdirSync(dir).some((f) => f.toLowerCase().endsWith('.glsl')); } catch (_) { return false; }
}
...
function quickSetupReady(config = CONFIG, options = {}) {
  ...
  // Aucun interpréteur Python n'est exigé : NetsuBoard n'en installe pas. Le runtime tient en deux
  // choses, ffmpeg et les shaders, et l'absence de l'une comme de l'autre rend l'upscale impossible.
  if (!exists(config.ffmpeg)) return false;
...
// Sonde du runtime. Chez NetsuRush elle lançait Python et importait torch ; ici il n'y a rien à
// lancer : le runtime est fait de FICHIERS (ffmpeg, ses DLL, les shaders), donc les regarder suffit.
// La forme du retour est conservée —  et l'écran d'installation la lisent telle quelle.
...
async function setupStatus() {
  ...
  return {
    ...
    items: [
      ...
      { id: 'shaders', label: `${labels.video} · shaders GPU`, done: shaders },
      { id: 'online', label: 'yt-dlp', done: online },
    ],
  };
}
