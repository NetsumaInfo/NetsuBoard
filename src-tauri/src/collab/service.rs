...
impl CollabService {
    ...
    pub async fn create_project(&self) -> Result<CreatedProject, CollabError> {
        let (reply, receive) = oneshot::channel();
        self.send(Command::CreateProject { reply }, receive).await
    }
...
async fn run_actor(
    ...
    let mut scenes: HashMap<String, ProjectId> = HashMap::new();
        ...
        match command {
            ...
            Command::Open { request, reply } => {
                let result = async {
                    ...
                    if request.scene_id.is_empty() || request.scene_id.len() > 128 {
                        return Err(CollabError::validation("invalid scene id"));
                    }
                    if let Some(bound) = scenes.get(&request.scene_id) {
                        if bound != &project_id {
                            return Err(CollabError::new(
                                CollabErrorCode::Conflict,
                                "scene is already bound to another collaborative project",
                            ));
                        }
                    }
                    if let Some(existing) = active.get(&project_id) {
                        if existing.scene_id != request.scene_id {
                            return Err(CollabError::new(
                                CollabErrorCode::Conflict,
                                "project is already open in another scene",
                            ));
                        }
                    }
                    if let Some(existing) = active.get_mut(&project_id) {
                        ...
                        return Ok(ProjectSession {
                            ...
                            scene_id: existing.scene_id.clone(),
                            role: existing.role,
                    ...
                    scenes.insert(request.scene_id, project_id.clone());
            ...
            Command::CreateProject { reply } => {
                let result = async {
                    ...
                    let response: Response = client
                        .mutation("projects:createProject", serde_json::json!({}))
                        .await?;
            ...
            Command::GrantKnownMedia {
                ...
                let result = (|| {
                    ...
                    super::blobs::issue_known_grant(parsed, &project.scene_id, &path).map_err(
