...
pub const ALPN_BLOB: &[u8] = b"netsuboard/blob/1";
...
/// Re-authorises a file from a saved scene, or an app-owned reference asset. Renderer input alone
/// is insufficient: the canonical path must already be present in trusted local application state.
...
mod path_tests {
    ...
    fn media_protocol_never_grants_cross_origin_cors() {
        ...
        let development = tauri::http::HeaderValue::from_static("http://localhost:1430");
