...
/// NetsuBoard file cannot be unprotected as an identity.
const DPAPI_ENTROPY: &[u8] = b"NetsuBoard.collab.device-identity.v1";
...
pub enum IdentityError {
    ...
    Internal(String),
    Dpapi(String),
    /// At-rest protection is unavailable, so no secret is written. NetsuBoard ships on Windows only;
...
/// identities. A second NetsuBoard process cannot exist: `tauri-plugin-single-instance` routes it
/// into the first one.
...
/// NetsuBoard's home, mirroring `core/config.js`: `NR_HOME`, else `%LOCALAPPDATA%\NetsuBoard`.
///
...
pub(crate) fn home_dir() -> PathBuf {
    ...
    if let Some(local) = std::env::var_os("LOCALAPPDATA") {
        return PathBuf::from(local).join("NetsuBoard");
    }
    let profile = std::env::var_os("USERPROFILE")
        .or_else(|| std::env::var_os("HOME"))
        .map(PathBuf::from)
        .unwrap_or_else(std::env::temp_dir);
    profile.join(".netsuboard")
}
