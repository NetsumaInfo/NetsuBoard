...
fn spawn_core(_app: &AppHandle) -> Option<Child> {
    let server = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("server.js");
    if !server.exists() {
        eprintln!("[netsuboard] core not found: {}", server.display());
        return None;
    }
    ...
    match command.spawn() {
        Ok(child) => {
            eprintln!("[netsuboard] core spawned: {}", server.display());
            Some(child)
        }
        Err(e) => {
            eprintln!("[netsuboard] failed to spawn core (is node on PATH?): {e}");
            None
        }
    }
}
...
fn spawn_core(app: &AppHandle) -> Option<Child> {
        ...
        .spawn()
    {
        Ok(child) => {
            log_core(&format!(
                "core service started (pid {}, port {port})",
                child.id()
            ));
            Some(child)
        }
