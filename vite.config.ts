...
export default defineConfig({
  ...
  server: {
    ...
    // MAIS le devUrl de tauri.conf.json reste « http://localhost:1430 » (PAS l'IP) : la WebView2
    // résout localhost → 127.0.0.1 (où Vite écoute) et l'ORIGINE du document devient
    // http://localhost:1430. C'est REQUIS pour les embeds YouTube : l'IFrame Player REFUSE une
    // origine en IP nue (127.0.0.1) → « Vidéo non disponible » ; il accepte « localhost ».
    //
    // Port 1430 et non 1420 : NetsuRush tourne en permanence sur 1420 avec `strictPort`, donc les
    // deux serveurs de développement doivent pouvoir vivre en même temps.
    host: "127.0.0.1",
    port: 1430,
    strictPort: true,
  },
