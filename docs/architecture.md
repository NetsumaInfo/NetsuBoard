...
core/server.js        Node service, HTTP on 127.0.0.1, headless, port picked at launch
  ...
  ├─ ytstream.js      /ytstream — YouTube stream relayed through yt-dlp
  ├─ appstatic.js     /app — the built renderer, served from resources in release
  ├─ rpc.js           POST /rpc + SSE /events
  ├─ reference.js     board scenes and assets (node:sqlite, JSON-file fallback)
  ├─ netsu.js + netsu/   `.netsu` project container (SQLite, chunked blobs)
  ├─ ffmpeg.js / thumbs.js / proxy.js / extract.js   probe, thumbnails, proxies, downloads
  ├─ shaderUpscale.js + turbo.js   libplacebo GLSL upscaling (Vulkan)
...
`App.tsx` renders **one page**: `ReferencePanel`. There is no sidebar, no tabs and no sub-navigation — there is a single module, and giving it a navigation would be furnishing empty space. The title bar is a drag strip with the brand mark and the window controls, and it must exist on the **first** render: the Tauri window is frameless, so without it the window cannot be moved or closed while the app is starting.

`App.tsx` also serves the **detached board**. The second Tauri `WebviewWindow` (label `reference`, frameless, always-on-top) loads the **same renderer** at hash `#reference`; `IS_REFERENCE_WINDOW` detects it and renders `ReferenceWindow` — the board bare, with no toolbar and no icons, every action in the context menu.

Settings are a **dialog** (`components/reference/BoardSettings.tsx`) with four tabs: Look, Media, Behavior, Keys. `src/features/settings/nav.ts` still describes NetsuRush's nine settings pages; nothing renders them, and only `store/shell.ts` still imports its types.

## Store

`src/store/` is a zustand store split into typed slices, assembled in `index.ts` (exported as `useApp`). `shell.ts` holds window-level state; board state lives in the reference components and their hooks. Most other slices are inherited from NetsuRush and unused by the board.
...
- **No JS animation on the board or on any grid.** GSAP (permanent rAF ticker) and framer-motion `layout`/`AnimatePresence` (projection loop) both saturated the main thread and GPU and competed with video decoding. GSAP was uninstalled. Use CSS and `content-visibility`.
- Off-screen items are culled (`useBoardCulling.ts`), never virtualised away — the board keeps its DOM and skips paint.
- A play-slot cap limits simultaneous `<video>` elements.
- Pause the `<video>` **element**; never gate its mounting on a scroll or visibility flag — that breaks autoplay (tried, broken).
